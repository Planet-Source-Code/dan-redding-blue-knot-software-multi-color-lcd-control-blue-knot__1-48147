Attribute VB_Name = "modResByte"
Option Explicit

'This code is an adaptation of code by 'Kamilche'
'http://www.planet-source-code.com/vb/scripts/ShowCode.asp?txtCodeId=29004&lngWId=1
'I replaced his function that loads a byte stream from a file (commented out below)
'with one that loads it from the resource file, which allows me to store GIFs, JPGs,
'WMFs or whatever else I want in custom resources.
...
'This part is the only part that is 'mine'
Public Function LoadResPic(strType As String, lID As Long) As StdPicture
Dim B() As Byte, pic As StdPicture
    B = LoadResData(lID, strType)
    Set LoadResPic = PictureFromByteStream(B)
End Function
